...
const  SYMNUM=  44;  /* SIZE IF SYMBOL TABLE*/
...
typedef enum  { NUL, IDENT, NUMBER, PLUS, MINUS, TIMES,
				...
				PLUSEQL, MINUSEQL, MULTIPLYEQL, DIVIDEEQL, PLUSPLUS, MINUSMINUS
        } SYMBOL;
...
void GetSym() {
  ...
  if (CH>='A' && CH<='Z') { /*ID OR RESERVED WORD*/
	...
	if (i-1 > J) SYM=WSYM[K];
      	...
      	if (SYM==ELSESYM) {
		Form1->printfs("else symbol");
	} else if (SYM==FORSYM) {
		Form1->printfs("for symbol");
	} else if (SYM==STEPSYM) {
		Form1->printfs("step symbol");
	} else if (SYM==UNTILSYM) {
		Form1->printfs("until symbol");
	} else if (SYM==DOSYM) {
		Form1->printfs("do symbol");
	} else if (SYM==RETURNSYM) {
		Form1->printfs("return symbol");
	}
  }
  else
    if (CH>='0' && CH<='9') { /*NUMBER*/
	  ...
	  if (K>NMAX) Error(30);
		...
		if (CH=='=') {
			SYM = PLUSEQL;
			Form1->printfs("plus equal symbol");
			GetCh();
		} else if (CH=='+') {
			SYM = PLUSPLUS;
			Form1->printfs("plus plus symbol");
			GetCh();
		} else {
			SYM = PLUS;
		}
		...
		if (CH=='=') {
			SYM = MINUSEQL;
            Form1->printfs("minus equal symbol");
			GetCh();
		} else if (CH=='-') {
			SYM = MINUSMINUS;
            Form1->printfs("minus minus symbol");
			GetCh();
		} else {
			SYM = MINUS;
		}
		...
		if (CH=='=') {
			SYM = MULTIPLYEQL;
			Form1->printfs("multiply equal symbol");
			GetCh();
		} else {
			SYM = TIMES;
		}
		...
		if (CH=='=') {
			SYM = DIVIDEEQL;
			Form1->printfs("divide equal symbol");
			GetCh();
		} else {
			SYM = SLASH;
		}
...
void STATEMENT(SYMSET FSYS,int LEV,int &TX) {   /*STATEMENT*/
  ...
  switch (SYM) {
	...
	case IFSYM:
		...
		CX1=CX; //用于地址回填
		GEN(JPC,0,0); //若栈顶为0（条件不满足），则转移至地址A。此处A=0，这是一个暂时的值。CX++
		// STATEMENT(FSYS,LEV,TX);  CODE[CX1].A=CX; 为了增加else语句而删除此行
		
		//以下新增
		STATEMENT(SymSetUnion(SymSetNew(ELSESYM),FSYS),LEV,TX); //生成语句S1 CX++
		if (SYM==ELSESYM) { //判断有无else标识符
			GetSym();
			CX2 = CX;
			GEN(JMP,0,0); //生成无条件跳转指令，因为到当前地址时，S1必定已经执行完，然后需要跳转end。CX++，此时CX是位于存放S2地址
			CODE[CX1].A = CX;
			STATEMENT(FSYS,LEV,TX); //生成语句S2 CX++ 此时的CX是结束地址
			CODE[CX2].A = CX;
		} else {
			CODE[CX1].A = CX; //没有检测到else标识符，则不满足条件时跳转end
		}
		break;
        //以上新增           
	case BEGINSYM:
    ...
    switch (I.F) {
	  ...
	  case OPR:
	    switch (I.A) { /*OPERATOR*/
	      ...
	      case 16: T++;  S[T]=InputBox("??????","????????????????o", 0).ToInt();
...
void __fastcall TForm1::ButtonRunClick(TObject *Sender) {
  ...
  strcpy(KWORD[ 7],"FOR");
  strcpy(KWORD[ 8],"IF");         strcpy(KWORD[9],"ODD");
  strcpy(KWORD[ 10],"PROCEDURE"); strcpy(KWORD[11],"PROGRAM");
  strcpy(KWORD[12],"READ");       strcpy(KWORD[13],"RETURN");
